...
-- MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=lakehouse&notebook=03-BI-Datawarehousing-fraud&demo_name=lakehouse-fsi-fraud-detection&event=VIEW">
...
-- MAGIC %md-sandbox
-- MAGIC
-- MAGIC ## Creating our Fraud Analysis Dashboard
-- MAGIC
-- MAGIC <img style="float: right; margin-left: 10px" width="600px" src="https://raw.githubusercontent.com/databricks-demos/dbdemos-resources/main/images/fsi/fraud-detection/lakehouse-fsi-fraud-dashboard.png" />
-- MAGIC
...
-- MAGIC
-- MAGIC The Dashboard has been loaded for you. Open the <a dbdemos-dashboard-id="fraud-detection" href='/sql/dashboardsv3/01ef3a4263bc1180931f6ae733179956' target="_blank">DBSQL FSI Fraud Dashboard</a> to start reviewing our existing Fraud pattern.
...
-- MAGIC %md
-- MAGIC
-- MAGIC # Taking our analysis one step further: Detecting Fraud
-- MAGIC
-- MAGIC Being able to run analysis on our past data already gives us a lot of insight. We can better understand our fraud pattern and quantify their impact.
-- MAGIC
-- MAGIC However, knowing that we had past fraud isn't enough. We now need to take it to the next level and build a predictive model to flag financial transaction as fraud in real time, improving our revenue and reducing risl.
-- MAGIC
