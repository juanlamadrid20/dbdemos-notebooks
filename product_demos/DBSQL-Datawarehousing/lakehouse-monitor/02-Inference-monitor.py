...
# MAGIC Create a dataset with predictions. The Inference Monitor uses a prediction column to calculate its statistics, a model version column that acts like a slice, and, optionally, a label column for the ground truth value. We specify the names for each of these columns when we create the Inference Monitor.
# MAGIC
...
# Create a monitor using a Timeseries profile type. After the initial refresh completes, you can view the autogenerated dashboard from the Quality tab of the table in Catalog Explorer. 
...
# MAGIC
# MAGIC The two aggregate metrics will be the sum of weights and the weighted sum of squared prediction errors. Then we'll divide the weighted sum by the sum of the weights to get the weighted mean squared error.
# MAGIC
